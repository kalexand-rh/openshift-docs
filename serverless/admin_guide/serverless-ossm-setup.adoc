...
Using {OSSMShortName} with {ServerlessProductName} enables developers to configure additional networking and routing options.

The {ServerlessOperatorName} provides Kourier as the default ingress for Knative. However, you can use {OSSMShortName} with {ServerlessProductName} whether Kourier is enabled or not. Integrating with Kourier disabled allows you to configure additional networking and routing options that the Kourier ingress does not support.

[IMPORTANT]
====
{ServerlessProductName} only supports the use of {OSSMName} functionality that is explicitly documented in this guide, and does not support other undocumented features.
====

// without kourier
[id="serverless-ossm-setup-native"]
== Integrating {OSSMShortName} with {ServerlessProductName} natively

Integrating {OSSMShortName} with {ServerlessProductName} natively, without Kourier, allows you to use additional networking and routing options that are not supported by the default Kourier ingress, such as mTLS functionality.
