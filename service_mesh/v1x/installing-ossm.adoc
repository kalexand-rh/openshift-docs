...
Installing the {OSSMShortName} involves installing the OpenShift Elasticsearch, Jaeger, Kiali and {OSSMShortName} Operators, creating and managing a `ServiceMeshControlPlane` resource to deploy the control plane, and creating a `ServiceMeshMemberRoll` resource to specify the namespaces associated with the {OSSMShortName}.
...
[NOTE]
====
Multi-tenant control plane installations are the default configuration starting with {OSSMName} 1.0.
====

[NOTE]
====
The {OSSMShortName} documentation uses `istio-system` as the example project, but you can deploy the service mesh to any project.
====

== Prerequisites
* Follow the xref:../../service_mesh/v1x/preparing-ossm-installation.adoc#preparing-ossm-installation-v1x[Preparing to install {OSSMName}] process.
* An account with the `cluster-admin` role.

The {OSSMShortName} installation process uses the link:https://operatorhub.io/[OperatorHub] to install the `ServiceMeshControlPlane` custom resource definition within the `openshift-operators` project. The {OSSMName} defines and monitors the `ServiceMeshControlPlane` related to the deployment, update, and deletion of the control plane.

Starting with {OSSMName} {OSSMVersion}, you must install the OpenShift Elasticsearch Operator, the Jaeger Operator, and the Kiali Operator before the {OSSMName} Operator can install the control plane.
...
* xref:../../service_mesh/v1x/prepare-to-deploy-applications-ossm.adoc#deploying-applications-ossm-v1x[Prepare to deploy applications] on {OSSMName}.
