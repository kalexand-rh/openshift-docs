...
When the {OSSMShortName} Operator deploys the `ServiceMeshControlPlane` resource, it can also create the resources for distributed tracing. {OSSMShortName} uses Jaeger for distributed tracing.
