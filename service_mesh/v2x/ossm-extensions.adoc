...
You can use WebAssembly extensions to add new features directly into the {OSSMName} proxies, allowing you to move even more common functionality out of your applications, and implement them in a single language that compiles to WebAssembly bytecode.
...
{OSSMName} extensions are link:https://www.envoyproxy.io/docs/envoy/v1.16.0/intro/arch_overview/http/http_filters#arch-overview-http-filters[Envoy HTTP Filters], giving them a wide range of capabilities:
...
There are two parts to writing a {OSSMName} extension: you'll have to write your extension using an SDK that exposes the link:https://github.com/proxy-wasm/spec[proxy-wasm API] and compile it to a WebAssembly module, and then package it into a container.

.Supported languages

You can use any language that compiles to WebAssembly bytecode to write a {OSSMName} extension, but the following languages have existing SDKs that expose the proxy-wasm API so that it can be consumed directly.
