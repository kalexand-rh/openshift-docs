...
This section describes how to identify and resolve common problems in {OSSMName}. Use the following sections to help troubleshoot and debug problems when deploying {OSSMName} on {product-title}.
...
The Service Mesh _control plane_ is composed of Istiod, which consolidates several previous control plane components (Citadel, Galley, Pilot) into a single binary. Deploying the `ServiceMeshControlPlane` also creates the other components that make up {OSSMName} as described in the xref:../../service_mesh/v2x/ossm-architecture.adoc#ossm-architecture_ossm-architecture[architecture] topic.
...
{OSSMName} relies on a proxy sidecar within the application’s pod to provide service mesh capabilities to the application.
...
For prompt support, supply diagnostic information for both {product-title} and {OSSMName}.
