...
The 3scale Istio Adapter is an optional adapter that allows you to label a service running within the {OSSMName} and integrate that service with the 3scale API Management solution.
It is not required for {OSSMName}.

[IMPORTANT]
====
You can only use the 3scale Istio adapter with {OSSMName} versions 2.0 and below. The Mixer component was deprecated in release 2.0 and removed in release 2.1. For {OSSMName} versions 2.1.0 and later you should use the xref:../../service_mesh/v2x/ossm-threescale-webassembly-module.adoc[3scale WebAssembly module].
