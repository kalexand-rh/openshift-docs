...
An installation of {OSSMName} differs from an installation of Istio in multiple ways. The modifications to {OSSMName} are sometimes necessary to resolve issues, provide additional features, or to handle differences when deploying on {product-title}.
...
.{OSSMName} matching request headers by using regular expressions
[source,yaml]
----
apiVersion: "rbac.istio.io/v1alpha1"
kind: ServiceRoleBinding
...
* Deployment of TLS certificates using the Secret Discovery Service (SDS) functionality of Istio is not currently supported in {OSSMName}. The Istio implementation depends on a nodeagent container that uses hostPath mounts.
