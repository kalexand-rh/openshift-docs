...
When the {OSSMShortName} Operator creates the `ServiceMeshControlPlane` it also processes the Kiali resource. The Kiali Operator then uses this object when creating Kiali instances.
...
|dashboard
   viewOnlyMode
|This parameter enables/disables view-only mode for the Kiali console.  When view-only mode is enabled, users cannot use the console to make changes to the {OSSMShortName}.
|`true`/`false`
|`false`
...
When you install Kiali and Grafana as part of {OSSMName} the Operator configures the following by default:
...
When you install Kiali and Jaeger as part of {OSSMName} the Operator configures the following by default:
