...
To install {OSSMName}, install following Operators in this order. Repeat the procedure for each Operator.
...
.. For the {JaegerName}, Kiali, and {OSSMName} Operators, accept the defaults.
+
The {JaegerName}, Kiali and {OSSMName} Operators are installed in the `openshift-operators` namespace. The OpenShift Elasticsearch Operator is installed in the `openshift-operators-redhat` namespace.
