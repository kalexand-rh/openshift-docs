...
These limitations exist in {OSSMName}:

* {OSSMName} does not yet support link:https://issues.redhat.com/browse/MAISTRA-1314[IPv6], as it is not yet fully supported by the upstream Istio project.
...
== {OSSMShortName} known issues

These are the known issues in {OSSMName}:

* link:https://github.com/istio/istio/issues/14743[Istio-14743] Due to limitations in the version of Istio that this release of {OSSMName} is based on, there are several applications that are currently incompatible with {OSSMShortName}. See the linked community issue for details.
...
* link:https://issues.redhat.com/browse/MAISTRA-2692[MAISTRA-2692] With Mixer removed, custom metrics that have been defined in {OSSMShortName} 2.0.x cannot be used in 2.1. Custom metrics can be configured using `EnvoyFilter`. Red Hat is unable to support `EnvoyFilter` configuration except where explicitly documented. This is due to tight coupling with the underlying Envoy APIs, meaning that backward compatibility cannot be maintained.
...
* link:https://issues.redhat.com/browse/MAISTRA-1314[MAISTRA-1314] {OSSMName} does not yet support IPv6.
