...
When the {OSSMShortName} Operator creates the `ServiceMeshControlPlane` resource it can also create the resources for distributed tracing. {OSSMShortName} uses Jaeger for distributed tracing.
...
If you are deploying {OSSMShortName} and Jaeger in a production environment you must change the template to the `production-elasticsearch` template, which uses Elasticsearch for Jaeger's storage needs.
...
|tracing:
  enabled:
|This parameter enables/disables tracing in {OSSMShortName}. Jaeger is installed by default.
|`true`/`false`
|`true`
|
