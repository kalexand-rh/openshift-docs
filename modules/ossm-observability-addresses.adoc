...
{OSSMName} provides the following consoles to view your service mesh data:

* *Kiali console* - Kiali is the management console for {OSSMName}.
...
* *Prometheus console* - {OSSMName} uses Prometheus to store telemetry information from services.
...
----
+
. To get the routes for the various {OSSMName} consoles, run the folowing command:
+
[source,terminal]
----
$ oc get routes
