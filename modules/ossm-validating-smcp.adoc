...
When you create the Service Mesh control plane, the {OSSMShortName} Operator uses the parameters that you have specified in the `ServiceMeshControlPlane` resource file to do the following:
...
[NOTE]
====
You view the Kiali components under the Kiali Operator, not the {OSSMShortName} Operator.
====
+
...
[NOTE]
====
You view the Jaeger components under the {JaegerName} Operator and the Elasticsearch components under the Elasticsearch Operator, not the {OSSMShortName} Operator.
====
+
