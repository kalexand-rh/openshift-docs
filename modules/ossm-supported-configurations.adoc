...
The following configurations are supported for the current release of {OSSMName}:
...
[NOTE]
====
Red Hat OpenShift Online is not supported for {OSSMName}.
====

* This release of {OSSMName} is only available on {product-title} x86_64, IBM Z, and IBM Power Systems.
** IBM Z is only supported on {product-title} 4.6 and later.
** IBM Power Systems is only supported on {product-title} 4.6 and later.
* Configurations where all {OSSMShortName} components are contained within a single {product-title} cluster. {OSSMName} does not support management of microservices that reside outside of the cluster within which {OSSMShortName} is running.
...
{OSSMName} supports the following network configurations.

* OpenShift-SDN
* OVN-Kubernetes is supported on {product-title} 4.7.32+, {product-title} 4.8.12+, and {product-title} 4.9+.
* Third-Party Container Network Interface (CNI) plug-ins that have been certified on {product-title} and passed {OSSMShortName} conformance testing. See link:https://access.redhat.com/articles/5436171[Certified OpenShift CNI Plug-ins] for more information.
