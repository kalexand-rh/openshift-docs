...
Configure your service mesh by adding your own traffic configuration to {OSSMName} with a custom resource definitions in a YAML file.

[id="ossm-routing-traffic-management-vs_{context}"]
== Traffic management with virtual services

You can route requests dynamically to multiple versions of a microservice through {OSSMName} with a virtual service. With virtual services, you can:
...
Requests are routed to services within a service mesh with virtual services. Each virtual service consists of a set of routing rules that are evaluated in order. {OSSMName} matches each given request to the virtual service to a specific real destination within the mesh.

Without virtual services, {OSSMName} distributes traffic using round-robin load balancing between all service instances. With a virtual service, you can specify traffic behavior for one or more hostnames. Routing rules in the virtual service tell {OSSMName} how to send the traffic for the virtual service to appropriate destinations. Route destinations can be versions of the same service or entirely different services.
...
The `destination` field in the route section specifies the actual destination for traffic that matches this condition. Unlike the virtual service's host, the destination's host must be a real destination that exists in the {OSSMName} service registry. This can be a mesh service with proxies or a non-mesh service added using a service entry. In this example, the hostname is a Kubernetes service name:
...
By default, {OSSMName} uses a round-robin load balancing policy, where each service instance in the pool gets a request in turn. {OSSMName} also supports the following models, which you can specify in destination rules for requests to a particular service or service subset.
...
Unlike other mechanisms for controlling traffic entering your systems, such as the Kubernetes Ingress APIs, {OSSMName} gateways allow you use the full power and flexibility of traffic routing. The {OSSMName} gateway resource can layer 4-6 load balancing properties such as ports to expose and configure {OSSMName} TLS settings. Instead of adding application-layer traffic routing (L7) to the same API resource, you can bind a regular {OSSMName} virtual service to the gateway and manage gateway traffic like any other data plane traffic in a service mesh.
...
A service entry adds an entry to the service registry that {OSSMName} maintains internally. After you add the service entry, the Envoy proxies send traffic to the service as if it is a service in your mesh. Service entries allow you to do the following:
...
[NOTE]
====
Add services from a different cluster to the mesh to configure a multicluster {OSSMName} mesh on Kubernetes.
====

.Service entry examples
The following example mesh-external service entry adds the `ext-resource` external dependency to the {OSSMName} service registry:
