...
////
{OSSMName} provides a number of key capabilities uniformly across a network of services:
...
== Component versions included in {OSSMName} version {OSSMVersion}
...
== New features {OSSMName} 2.1.1

This release of {OSSMName} addresses Common Vulnerabilities and Exposures (CVEs) and bug fixes.
...
[NOTE]
====
When you disable `spec.security.manageNetworkPolicy` {OSSMName} will not create *any* `NetworkPolicy` objects.  The system administrator is responsible for managing the network and fixing any issues this might cause.
====
...
. Click the {OSSMName} Operator. In the *Istio Service Mesh Control Plane* column, click the name of your `ServiceMeshControlPlane`, for example `basic-install`.
...
== New features and enhancements {OSSMName} 2.1

This release of {OSSMName} adds support for Istio 1.9.8, Envoy Proxy 1.17.1, Jaeger 1.24.1, and Kiali 1.36.5 on {product-title} 4.6 EUS, 4.7, 4.8, and 4.9.
...
The OVN-Kubernetes Container Network Interface (CNI) was previously introduced as a Technology Preview feature in {OSSMName} 2.0.1 and is now generally available in {OSSMName} 2.1 and 2.0.x for use on {product-title} 4.7.32, {product-title} 4.8.12, and {product-title} 4.9.
...
{OSSMShortName} 2.1 is based on Istio 1.9, which brings in a large number of new features and product enhancements. While the majority of Istio 1.9 features are supported, the following exceptions should be noted:
...
The amount of time {OSSMName} uses to prune old resources at the end of every `ServiceMeshControlPlane` reconciliation has been reduced. This results in faster `ServiceMeshControlPlane` deployments, and allows changes applied to existing SMCPs to take effect more quickly.
...
* {OSSMShortName} troubleshooting functionality
...
== New features {OSSMName} 2.0.7

This release of {OSSMName} addresses Common Vulnerabilities and Exposures (CVEs) and bug fixes.

== {OSSMName} on {product-dedicated} and Microsoft Azure Red Hat OpenShift

{OSSMName} is now supported through {product-dedicated} and Microsoft Azure Red Hat OpenShift.

== New features {OSSMName} 2.0.6

This release of {OSSMName} addresses Common Vulnerabilities and Exposures (CVEs) and bug fixes.

== New features {OSSMName} 2.0.5

This release of {OSSMName} addresses Common Vulnerabilities and Exposures (CVEs) and bug fixes.

== New features {OSSMName} 2.0.4
...
[NOTE]
====
The {OSSMName} configuration location for path normalization is different from the Istio configuration.
====
...
To configure path normalization for {OSSMName}, specify the following in your `ServiceMeshControlPlane`. Use the configuration examples to help determine the settings for your system.
...
== New features {OSSMName} 2.0.2

This release of {OSSMName} adds support for IBM Z and IBM Power Systems. It also addresses Common Vulnerabilities and Exposures (CVEs) and bug fixes.

== New features {OSSMName} 2.0.1

This release of {OSSMName} addresses Common Vulnerabilities and Exposures (CVEs) and bug fixes.

== New features {OSSMName} 2.0

This release of {OSSMName} adds support for Istio 1.6.5, Jaeger 1.20.0, Kiali 1.24.2, and the 3scale Istio Adapter 2.0 and {product-title} 4.6.
