...
= Integrating {OSSMShortName} with {ServerlessProductName}

You can integrate {OSSMShortName} with {ServerlessProductName} without using Kourier by completing the following procedure.

.Prerequisites

* You have installed {OSSMName}. {ServerlessProductName} with {OSSMShortName} only is supported for use with {OSSMName} version 2.0.5 or higher.
...
. Create the necessary gateways so that {OSSMShortName} can accept traffic:
+
...
<2> The `knative-local-gateway` serves HTTP traffic. Using HTTP means that traffic coming from outside of {OSSMShortName}, but using an internal hostname, such as `example.default.svc.cluster.local`, is not encrypted. You can set up encryption for this path by creating another wildcard certificate and an additional gateway that uses a different `protocol` spec.
+
...
<3> Injects {OSSMShortName} sidecars into the Knative service pods.
